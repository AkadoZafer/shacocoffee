                            ...
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold uppercase ${order.status === 'Onaylandı' ? 'bg-green-500/10 text-green-500' : 'bg-orange-500/10 text-orange-500'}`}>
                                            {order.status}
                                        </span>
                                    </div>
                                    <p className={`text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>{new Date(order.date || order.timestamp).toLocaleString('tr-TR')}</p>
                                </div>
                            ...
                            <div className={`space-y-2 pt-3 border-t ${isDark ? 'border-zinc-800' : 'border-zinc-100'}`}>
                                {order.items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between text-sm">
                                        <span>{item.name} <span className="text-zinc-500 text-xs">x{item.quantity}</span></span>
