        ...
        <div className={`min-h-screen p-6 pb-32 transition-colors duration-300 ${isDark ? 'bg-black text-white' : 'bg-zinc-50 text-zinc-900'}`}>
            ...
            <AnimatePresence>
                {showSuccess && (
                    ...
                    >
                        <Check size={16} /> +₺{loadedAmount} yüklendi!
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            >
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-zinc-500 text-[10px] font-bold tracking-[0.2em]">TOPLAM BAKİYE</p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-4">
                        ...
                        <span className="text-[9px] text-zinc-600 font-bold tracking-[0.2em]">SHACO COFFEE</span>
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className={`text-[10px] font-bold tracking-[0.15em] ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>HIZLI YÜKLE</p>
                </div>
                ...
                <div className={`flex items-center gap-3 p-3 rounded-xl mb-4 ${isDark ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-zinc-200 shadow-sm'}`}>
                    ...
                    <input
                        ...
                        className={`flex-1 bg-transparent outline-none text-sm font-display ${isDark ? 'text-white placeholder:text-zinc-700' : 'text-zinc-900 placeholder:text-zinc-400'}`}
                    />
                </div>
                ...
                <button
                    ...
                    className={`w-full py-4 rounded-xl font-bold text-[13px] flex items-center justify-center gap-2 transition active:scale-[0.97] ${activeAmount > 0
                        ? 'bg-shaco-red text-white shadow-lg shadow-red-500/20'
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-8">
                <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className={`text-[10px] font-bold tracking-[0.15em] ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>KARTLARIM</h3>
                    </div>
                ...
                <div className="space-y-2.5">
                    ...
                    <div className={`p-4 rounded-2xl flex items-center gap-4 transition active:scale-[0.98] cursor-pointer ${isDark ? 'bg-zinc-900 border border-zinc-800 hover:border-zinc-700' : 'bg-white border border-zinc-200 shadow-sm hover:shadow-md'}`}>
                        ...
                        <div className="flex-1">
                            ...
                            <p className={`text-[9px] font-mono tracking-widest mt-0.5 ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>Visa • 12/28</p>
                        </div>
                    ...
                    <div className={`p-4 rounded-2xl flex items-center gap-4 transition active:scale-[0.98] cursor-pointer ${isDark ? 'bg-zinc-900 border border-zinc-800 hover:border-zinc-700 opacity-60 hover:opacity-100' : 'bg-white border border-zinc-200 shadow-sm hover:shadow-md opacity-70 hover:opacity-100'}`}>
                        <div className={`w-10 h-7 rounded flex items-center justify-center font-bold italic text-[9px] flex-shrink-0 ${isDark ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : 'bg-zinc-100 text-zinc-500 border border-zinc-200'}`}>
                            MC
                        </div>
                        <div className="flex-1">
                            <p className={`font-display font-bold tracking-widest text-[14px] ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>•••• 8831</p>
                            <p className={`text-[9px] font-mono tracking-widest mt-0.5 ${isDark ? 'text-zinc-700' : 'text-zinc-400'}`}>Mastercard • 09/25</p>
                        </div>
                ...
                {showAuthPrompt && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-6" onClick={() => setShowAuthPrompt(false)}>
                        ...
                        >
                            ...
                            <p className={`text-sm mb-5 ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>Bakiye yüklemek için giriş yapmanız veya kayıt olmanız gerekiyor.</p>
