    ...
    const startCamera = async () => {
        try {
            ...
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                // Try to request permission through the Permissions API first
                if (navigator.permissions) {
                    try {
                        const result = await navigator.permissions.query({ name: 'camera' });
                        if (result.state === 'denied') {
                            setScanError('Kamera izni reddedildi. Lütfen Ayarlar > Uygulamalar > Shaco Coffee > İzinler kısmından kamera iznini açın.');
                            setIsScanning(false);
                            setShowManualInput(true);
                            return;
                        }
                    } catch (permErr) {
                        // permissions API may not support 'camera' query
                    }
                }
                setScanError('Kamera bu cihazda kullanılamıyor. Manuel giriş kullanın.');
                setIsScanning(false);
                setShowManualInput(true);
                return;
            }
            ...
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                ...
                setScanError('Kamera bulunamadı. Manuel giriş kullanın.');
            } else {
                setScanError('Kamera açılamadı. Manuel giriş kullanın.');
            }
            setIsScanning(false);
            setShowManualInput(true);
        }
